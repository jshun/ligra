...
template <typename F>
static void par_for(long start, long end, F f,
			 long granularity = 0,
...
template <typename F>
inline void par_for(long start, long end, F f,
			 long granularity,
			 bool conservative) {
  if (granularity == 0)
    ...
    cilk_spawn par_for(start, mid, f, granularity);
    par_for(mid, end, f, granularity);
    cilk_sync;
  }
}
