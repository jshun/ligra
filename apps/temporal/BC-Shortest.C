...
/**** LIGRA app start ****/
/**
 * TemporalCentrality: Betweeness using Shortest temporal paths.
 *
 * See: Ligra's Betweeness Centrality app (BC.C).
 */
struct BC_Shortest_F {
  ...
  BC_Shortest_F(intE* _EarliestArrivalTime, intE* _Shortest, fType* _NumPaths,
  ...
  //
  // Here we have an implementation of betweeness centrality using *shortest*
  ...
  inline bool updateAtomic(uintE s, uintE d, intE edge_weight,
    ...
    volatile intE newDist;
    do {
      newDist = Shortest[s] + edge_weight;
      oldV = NumPaths[d];
      newV = oldV + NumPaths[s];
    } while (!(writeMin(&EarliestArrivalTime[d], edge_end_time) &&
               writeMin(&Shortest[d], newDist) &&
...
struct BC_Shortest_Back_F {
  ...
  BC_Shortest_Back_F(fType* _Dependencies, bool* _Visited,
...
struct BC_Shortest_Vertex_F {
  bool* Visited;
  BC_Shortest_Vertex_F(bool* _Visited) : Visited(_Visited) {}
...
struct BC_Shortest_Back_Vertex_F {
  bool* Visited;
  fType *Dependencies, *inverseNumPaths;
  BC_Shortest_Back_Vertex_F(bool* _Visited, fType* _Dependencies,
...
void Compute(graph<vertex>& GA, long start_vertex_id, time_t start_time,
  ...
  while (!Frontier.isEmpty()) {  // first phase
    ...
    vertexSubset output =
        edgeMap(GA, Frontier,
                BC_Shortest_F(EarliestArrivalTime, Shortest, NumPaths, Visited,
                              start_time, end_time));
    vertexMap(output, BC_Shortest_Vertex_F(Visited));  // mark visited
  ...
  vertexMap(Frontier,
            BC_Shortest_Back_Vertex_F(Visited, Dependencies, inverseNumPaths));

  // tranpose graph
  GA.transpose();
  for (long r = round - 2; r >= 0; r--) {  // backwards phase
    edgeMap(GA, Frontier,
            BC_Shortest_Back_F(Dependencies, Visited, start_time, end_time), -1,
            no_output);
    Frontier.del();
    Frontier = Levels[r];  // gets frontier from Levels array
    // vertex map to mark visited and update Dependencies scores
    vertexMap(Frontier, BC_Shortest_Back_Vertex_F(Visited, Dependencies,
                                                  inverseNumPaths));
  }
