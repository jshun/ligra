...
struct denseT {
...
#ifdef WEIGHTED
  inline bool srcTarg(const uintE &src, const uintE &target, const intE &weight,
  ...
  inline bool srcTarg(const uintE &src, const uintE &target, const intE &weight,
                      const intE &start_time, const intE &end_time,
                      const uintT &edgeNumber) {
    if (vs.isIn(target)) {
      auto m = f.update(target, src, weight, start_time, end_time);
      g(src, m);
    }
    return f.cond(src);
  }
#else
  inline bool srcTarg(const uintE &src, const uintE &target,
                      const uintT &edgeNumber) {
    if (vs.isIn(target)) {
...
struct denseForwardT {
  ...
  inline bool srcTarg(const uintE &src, const uintE &target, const intE &weight,
                      ...
                      const uintT &edgeNumber) {
    if (f.cond(target)) {
      auto m = f.updateAtomic(src, target, weight, start_time, end_time);
      g(target, m);
    }
    return true;
  }
...
struct sparseT {
  ...
  inline bool srcTarg(const uintE &src, const uintE &target, const intE &weight,
                      ...
                      const uintT &edgeNumber) {
    if (f.cond(target)) {
      auto m = f.updateAtomic(v, target, weight, start_time, end_time);
      g(target, o + edgeNumber, m);
    } else {
      g(target, o + edgeNumber);
    }
...
struct sparseTSeq {
  ...
  G g;
  size_t &k;
  sparseTSeq(F &_f, G &_g, uintT vP, uintT oP, size_t &_k)
      : f(_f), g(_g), v(vP), o(oP), k(_k) {}
#ifdef WEIGHTED
  inline bool srcTarg(const uintE &src, const uintE &target, const intE &weight,
                      const uintT &edgeNumber) {
    if (f.cond(target)) {
...
inline void decodeInNghBreakEarly(V *v, long i, VS &vertexSubset, F &f, G &g,
                                  bool parallel = 0) {
...
#elif WEIGHTED_TEMPORAL
  decodeWghTemporal(denseT<F, G, VS>(f, g, vertexSubset), nghArr, i,
                    v->getInDegree());
#else
...
#elif WEIGHTED_TEMPORAL
  decodeWghTemporal(denseForwardT<F, G>(f, g), nghArr, i, v->getOutDegree());
#else
...
#elif WEIGHTED_TEMPORAL
  decodeWghTemporal(sparseT<F, G>(f, g, i, o), nghArr, i, v->getOutDegree());
#else
...
inline size_t decodeOutNghSparseSeq(V *v, long i, uintT o, F &f, G &g) {
  ...
  size_t k = 0;
#ifdef WEIGHTED
  decodeWgh(sparseTSeq<F, G>(f, g, i, o, k), nghArr, i, v->getOutDegree(),
            false);
#elif WEIGHTED_TEMPORAL
  decodeWghTemporal(sparseTSeq<F, G>(f, g, i, o, k), nghArr, i,
                    v->getOutDegree(), false);
#else
...
#elif WEIGHTED_TEMPORAL
  decodeWghTemporal(sparseTCount<F>(f, i, ct), nghArr, i, v->getOutDegree(),
                    false);
#else
...
#elif WEIGHTED_TEMPORAL
  decodeWghTemporal(sparseTE<E, F, G>(f, g, i, o), nghArr, i,
                    v->getOutDegree());
#else
...
inline size_t packOutNgh(V *v, long i, P &pred, bool *bits, uintE *tmp1,
                         uintE *tmp2) {
  uchar *nghArr = v->getOutNeighbors();
  ...
  auto gen = [&](const uintE &ngh, const uintE &offset,
                 const Maybe<uintE> &val = Maybe<uintE>()) {
    tmp1[offset] = val.t;
  };
  copyOutNgh<V, uintE>(v, i, (uintT)0, f, gen);

  size_t new_deg = pbbs::filterf(tmp1, tmp2, original_deg,
...
struct compressedSymmetricVertex {
  ...
  intT getInNeighbor(intT j) const {
    std::cerr << "ERROR: called compressedSymmetricVertex::getInNeighbor!"
              << std::endl;
    return -1;
  }
  // should not be called
  intT getOutNeighbor(intT j) const {
    std::cerr << "ERROR: called compressedSymmetricVertex::getOutNeighbor!"
              << std::endl;
    return -1;
  }
...
#ifdef WEIGHTED
  // should not be called
  intT getInWeight(intT j) const {
    std::cerr << "ERROR: called compressedSymmetricVertex::getInWeight!"
              << std::endl;
    return -1;
  }
  // should not be called
  intT getOutWeight(intT j) const {
    std::cerr << "ERROR: called compressedSymmetricVertex::getOutWeight!"
              << std::endl;
    return -1;
  }
#elif WEIGHTED_TEMPORAL
  ...
  // should not be called
  intT getInStartTime(intT j) const {
    std::cerr << "ERROR: called compressedSymmetricVertex::getInStartTime!"
              << std::endl;
    return -1;
  }
  // should not be called
  intT getOutStartTime(intT j) const {
    std::cerr << "ERROR: called compressedSymmetricVertex::getOutStartTime!"
              << std::endl;
    return -1;
  }
  // should not be called
  intT getInEndTime(intT j) const {
    std::cerr << "ERROR: called compressedSymmetricVertex::getInEndTime!"
              << std::endl;
    return -1;
  }
  // should not be called
  intT getOutEndTime(intT j) const {
    std::cerr << "ERROR: called compressedSymmetricVertex::getOutEndTime!"
              << std::endl;
    return -1;
  }
#endif
  ...
  inline void decodeInNghBreakEarly(long i, VS &vertexSubset, F &f, G &g,
                                    bool parallel = 0) {
    decode_compressed::decodeInNghBreakEarly<compressedSymmetricVertex, F, G,
                                             VS>(this, i, vertexSubset, f, g,
                                                 parallel);
  }

  template <class F, class G>
  inline void decodeOutNgh(long i, F &f, G &g) {
    decode_compressed::decodeOutNgh<compressedSymmetricVertex, F, G>(this, i, f,
                                                                     g);
  }

  template <class F, class G>
  inline void decodeOutNghSparse(long i, uintT o, F &f, G &g) {
    decode_compressed::decodeOutNghSparse<compressedSymmetricVertex, F, G>(
        this, i, o, f, g);
  }

  template <class F, class G>
  inline size_t decodeOutNghSparseSeq(long i, uintT o, F &f, G &g) {
    return decode_compressed::decodeOutNghSparseSeq<compressedSymmetricVertex,
                                                    F, G>(this, i, o, f, g);
  }

  template <class E, class F, class G>
  inline void copyOutNgh(long i, uintT o, F &f, G &g) {
    decode_compressed::copyOutNgh<compressedSymmetricVertex, E, F, G>(this, i,
                                                                      o, f, g);
  }

  template <class F>
  inline size_t countOutNgh(long i, F &f) {
    return decode_compressed::countOutNgh<compressedSymmetricVertex, F>(this, i,
                                                                        f);
  }

  template <class P>
  inline size_t packOutNgh(long i, P &pred, bool *bits, uintE *tmp1,
                           uintE *tmp2) {
    return decode_compressed::packOutNgh<compressedSymmetricVertex, P>(
        this, i, pred, bits, tmp1, tmp2);
  }
};
...
struct compressedAsymmetricVertex {
  ...
  inline void decodeInNghBreakEarly(long i, VS &vertexSubset, F &f, G &g,
                                    bool parallel = 0) {
    decode_compressed::decodeInNghBreakEarly<compressedAsymmetricVertex, F, G,
                                             VS>(this, i, vertexSubset, f, g,
                                                 parallel);
  }
  ...
  template <class F, class G>
  inline void decodeOutNgh(long i, F &f, G &g) {
    decode_compressed::decodeOutNgh<compressedAsymmetricVertex, F, G>(this, i,
                                                                      f, g);
  }

  template <class F, class G>
  inline void decodeOutNghSparse(long i, uintT o, F &f, G &g) {
    decode_compressed::decodeOutNghSparse<compressedAsymmetricVertex, F, G>(
        this, i, o, f, g);
  }

  template <class F, class G>
  inline size_t decodeOutNghSparseSeq(long i, uintT o, F &f, G &g) {
    return decode_compressed::decodeOutNghSparseSeq<compressedAsymmetricVertex,
                                                    F, G>(this, i, o, f, g);
  }

  template <class E, class F, class G>
  inline void copyOutNgh(long i, uintT o, F &f, G &g) {
    decode_compressed::copyOutNgh<compressedAsymmetricVertex, E, F, G>(this, i,
                                                                       o, f, g);
  }

  template <class F>
  inline size_t countOutNgh(long i, F &f) {
    return decode_compressed::countOutNgh<compressedAsymmetricVertex, F>(this,
                                                                         i, f);
  }
  ...
  inline size_t packOutNgh(long i, P &pred, bool *bits, uintE *tmp1,
                           uintE *tmp2) {
    return decode_compressed::packOutNgh<compressedAsymmetricVertex, P>(
        this, i, pred, bits, tmp1, tmp2);
  }
};
