...
uintT encode_data(uintE d,  uchar** dPtr){
	uchar *dataPointer = *dPtr;
	uintT code;
//	cout << "difference: " << d << endl;
	if(d < (1 << 8)){
		...
		*dataPointer += 1;
		...
		memcpy(dataPointer, &d, 2);
		*dataPointer += 2;
		code = 1;
	}
	else if (d < (1 << 24)){
		memcpy(dataPointer, &d, 3);
		*dataPointer += 3;
		code = 2;
	}
	else{
		memcpy(dataPointer, &d, 4);
		*dataPointer += 4;
		code = 3;
	}
...
static uint8_t *svb_encode_scalar(const uint32_t *in, uchar* controlPointer, uchar* dataPointer, uintT countLeft){
	if (countLeft = 0){
		return dataPointer;
	}
	...
	for(uintT c = 0; c < countLeft; c++){
		...
		uintT val = in[c];
		uchar code = encode_data(val, &dataPointer);
		key |= code << shift;
		shift += 2;
	}
...
long sequentialCompressEdgeSet(uchar *edgeArray, long currentOffset, uintT degree, uintE vertexNum, uintE *savedEdges){
	if(degree > 0){
		uchar *controlPtr = edgeArray;
		uchar *dataPtr = edgeArray + ((degree + 3)/4);
		uintT count = degree/4;
		uintE difference[degree];
	//	uintE *diffPtr = &difference;
		intE preCompress = vertexNum - savedEdges[0];
		intE toCompress = abs(preCompress);
