...
inline void decodeInNghBreakEarly(vertex* v, long v_id, VS& vertexSubset, F& f,
                                  G& g, bool parallel = 0) {
  ...
  if (!parallel || d < 1000) {
    for (size_t j = 0; j < d; j++) {
      ...
      if (vertexSubset.isIn(ngh)) {
        ...
        auto m = f.update(ngh, v_id, v->getInWeight(j), v->getInStartTime(j),
                          v->getInEndTime(j));
#else
...
#elif WEIGHTED_TEMPORAL
        auto m = f.updateAtomic(ngh, v_id, v->getInWeight(j),
                                v->getInStartTime(j), v->getInEndTime(j));
#else
...
#elif WEIGHTED_TEMPORAL
      auto m = f.updateAtomic(i, ngh, v->getOutWeight(j), v->getOutStartTime(j),
                              v->getOutEndTime(j));
#else
...
#elif WEIGHTED_TEMPORAL
      if (f(vtx_id, ngh, v->getOutWeight(i), v->getOutStartTime(i),
            v->getOutEndTime(i)))
#else
      if (f(vtx_id, ngh))
#endif
...
#elif WEIGHTED_TEMPORAL
        if (f(vtx_id, ngh, v->getOutNeighbor(j), v->getOutStartTime(j),
              v->getOutEndTime(j)))
#else
        if (f(vtx_id, ngh))
#endif
...
#elif WEIGHTED_TEMPORAL
    E val = f(src, ngh, v->getOutWeight(j), v->getOutStartTime(j), v->getOutEndTime(j));
#else
    E val = f(src, ngh);
#endif
    g(ngh, o + j, val);
...
struct symmetricVertex {
  ...
  intE* getOutNeighbors() { return neighbors; }
  const intE* getOutNeighbors() const { return neighbors; }
  intE getInNeighbor(intT j) const { return neighbors[4 * j]; }
  intE getOutNeighbor(intT j) const { return neighbors[4 * j]; }
  intE getInWeight(intT j) const { return neighbors[4 * j + 1]; }
  intE getOutWeight(intT j) const { return neighbors[4 * j + 1]; }
  intE getInStartTime(intT j) const { return neighbors[4 * j + 2]; }
  intE getOutStartTime(intT j) const { return neighbors[4 * j + 2]; }
  intE getInEndTime(intT j) const { return neighbors[4 * j + 3]; }
  intE getOutEndTime(intT j) const { return neighbors[4 * j + 3]; }
  void setInNeighbor(uintT j, uintE ngh) { neighbors[4 * j] = ngh; }
  void setOutNeighbor(uintT j, uintE ngh) { neighbors[4 * j] = ngh; }
  void setInWeight(uintT j, intE wgh) { neighbors[4 * j + 1] = wgh; }
  void setOutWeight(uintT j, intE wgh) { neighbors[4 * j + 1] = wgh; }
  void setInStartTime(uintT j, intE st) { neighbors[4 * j + 2] = st; }
  void setOutStartTime(uintT j, intE st) { neighbors[4 * j + 2] = st; }
  void setInEndTime(uintT j, intE et) { neighbors[4 * j + 3] = et; }
  void setOutEndTime(uintT j, intE et) { neighbors[4 * j + 3] = et; }
  void setInNeighbors(intE* _i) { neighbors = _i; }
  void setOutNeighbors(intE* _i) { neighbors = _i; }
#else
  ...
  inline void decodeInNghBreakEarly(long v_id, VS& vertexSubset, F& f, G& g,
                                    bool parallel = 0) {
  ...
  inline void decodeOutNghSparse(long i, uintT o, F& f, G& g) {
    decode_uncompressed::decodeOutNghSparse<symmetricVertex, F>(this, i, o, f,
                                                                g);
  }
  ...
  inline size_t packOutNgh(long i, F& f, bool* bits, uintE* tmp1, uintE* tmp2) {
    return decode_uncompressed::packOutNgh<symmetricVertex, F>(this, i, f, bits,
                                                               tmp1);
  }
};
...
struct asymmetricVertex {
...
#elif WEIGHTED_TEMPORAL
  intE *inNeighbors, *outNeighbors;  // weight arrays just store more data
#else
  ...
  void setInWeight(uintT j, intE wgh) { inNeighbors[2 * j + 1] = wgh; }
  ...
  intE* getOutNeighbors() { return outNeighbors; }
  const intE* getOutNeighbors() const { return outNeighbors; }
  intE getInNeighbor(uintT j) const { return inNeighbors[4 * j]; }
  intE getOutNeighbor(uintT j) const { return outNeighbors[4 * j]; }
  intE getInWeight(uintT j) const { return inNeighbors[4 * j + 1]; }
  intE getOutWeight(uintT j) const { return outNeighbors[4 * j + 1]; }
  intE getInStartTime(uintT j) const { return inNeighbors[4 * j + 2]; }
  intE getOutStartTime(uintT j) const { return outNeighbors[4 * j + 2]; }
  intE getInEndTime(uintT j) const { return inNeighbors[4 * j + 3]; }
  intE getOutEndTime(uintT j) const { return outNeighbors[4 * j + 3]; }
  void setInNeighbor(uintT j, uintE ngh) { inNeighbors[4 * j] = ngh; }
  void setOutNeighbor(uintT j, uintE ngh) { outNeighbors[4 * j] = ngh; }
  void setInWeight(uintT j, intE wgh) { inNeighbors[4 * j + 1] = wgh; }
  void setOutWeight(uintT j, intE wgh) { outNeighbors[4 * j + 1] = wgh; }
  void setInStartTime(uintT j, intE st) { inNeighbors[4 * j + 2] = st; }
  void setOutStartTime(uintT j, intE st) { outNeighbors[4 * j + 2] = st; }
  void setInEndTime(uintT j, intE et) { inNeighbors[4 * j + 3] = et; }
  void setOutEndTime(uintT j, intE et) { outNeighbors[4 * j + 3] = et; }
  void setInNeighbors(intE* _i) { inNeighbors = _i; }
  void setOutNeighbors(intE* _i) { outNeighbors = _i; }
#else
  ...
  template <class F, class G>
  inline void decodeOutNghSparse(long i, uintT o, F& f, G& g) {
    decode_uncompressed::decodeOutNghSparse<asymmetricVertex, F>(this, i, o, f,
                                                                 g);
  }
